  ...
  <div class="">
    <div class="change-page">
      <div class="catalog-page change-page-common">
        ...
        <span :class="{changePageActive: changePageIndex === 1}" @click="changePage(event,1)">菜单</span>
      </div>
    ...
    <div v-show="changePageIndex === 2" class="mobile-sidebar-catalog sidebar-nav">
      <div style="overflow: hidden" :key="itemLevel1Index" v-for="(itemLevel1,itemLevel1Index) in navbarLinks">
        <!--这是顶部导航的文章目录 一级标题-->
        <div v-if="itemLevel1.children" id="catalog-single" :class="getCatalogOpenStatus(itemLevel1Index)"
          ...
          <div v-if="itemLevel1.children" class="page-catalog-parent">
            ...
            <div v-if="itemLevel1.children" @click="changeCurrentLevel1Active(event,itemLevel1Index)"
              class="catalog-page-spread">
                ...
                <div class="page-catalog-children-level3-parent">
                  <div :style="setLevel3Style(itemLevel1Index)" class="page-catalog-children-level3">
                    <div :key="itemLevel3Index" v-for="(item,itemLevel3Index) in itemLevel2.children"
                      ...
                      <span v-if="getIsOuterLink(item.link)">
                        <a target="_blank" v-if="item.link" :da="item.link" :data="getIsOuterLink(item.link)"
                          :href="item.link">
                          <span>{{item.text}}</span>
                        </a>
                      </span>
    ...
    <!-- 这是是目录 -->
    <catalog @get-current-catalog-object="getCurrentCatalogObject" class="mobile-sidebar-catalog"
      :is-show-catalog="true" v-show="changePageIndex === 3" />
  ...
  computed: {
    ...
    setHomeNavIco() {
      return (href) => {
        ...
        return (
          "--aurora-sidebar-nav-other-ico: '\\" +
          icoArr[this.getRandomInt(0, icoArr.length)] +
          "'"
        );
      };
    },
  ...
  methods: {
    ...
    changeCurrentLevel1Active(e, index) {
      if (this.catalogOpenStatus.openStatus) {
        //是打开状态
        if (this.catalogOpenStatus.index === index) {
          this.catalogOpenStatus.openStatus =
            !this.catalogOpenStatus.openStatus;
        }
      } else {
